...
model.out <- data.frame(site   = rep(rep(climate[,1], ncol(wa.res$bias)),5),
                        ...
                        init = rep(climate[,10], ncol(wa.res$bias), 5))
...
samp.set <- sapply(c('MAT', 'WA', 'WAPLS', 'rFor', 'BRT'), function(x){
  set.len <- sum(model.out$model == x)
  if( set.len > 5000) set.len <- 5000
  sample(which(model.out$model == x), set.len)
})
...
ggplot(model.samp, aes(x = dissim, y = bias, group = site, color = init)) +
  geom_path(alpha = 0.1) +
  theme_bw() +
  facet_wrap(~model, scales='free_y', nrow = 1) +
  scale_x_continuous(expand=c(0,0)) +
...
ggplot(model.out, aes(x = dissim, y = variance, group = site, color = init)) +
