...
```{r SiteLocations, fig.width=7, fig.height=6, fig.cap='Figure: North American site locations', echo=FALSE, message=FALSE, warning=FALSE}
map <- map_data('world')
...
```{r MinMaxDissim, fig.width=7, fig.height=6, fig.cap='Figure: North American site locations', echo=FALSE, message=FALSE, warning=FALSE}
...
ggplot(dissim.scores) + 
  ...
  annotate("segment", x=0.48, 
           xend = -.32, y = 0, yend = 1, size=3, color='red') +
...
```{r SiteMDS, fig.width=7, fig.height=6, fig.cap='Figure: North American site MDS scores', echo=FALSE, message=FALSE, warning=FALSE}
...
mat.df <- data.frame(site = rep(climate[,1], ncol(mat.res$bias)),
                     preds = as.vector(mat.res$mean_prediction),
                     dissim = rep(seq(0, 1, by=0.01),each = nrow(clim)),
                     model = 'MAT', 
                     init = rep(climate[,10], ncol(mat.res$bias)))

model.out <- data.frame(site   = rep(rep(climate[,1], ncol(wa.res$bias)),2),
                        preds  = c(as.vector(wa.res$mean_prediction),
                                   as.vector(wapls.res$mean_prediction)),
                        dissim = rep(rep(vals, each=nrow(wa.res$bias)),2),
                        model  = rep(c('WA', 'WAPLS'), each = length(wa.res$bias)),
                        init = rep(climate[,10], ncol(wa.res$bias)))

#  Temporary fix until we re-run wa.re and wapls.res
model.out <- rbind(mat.df, model.out)
model.out <- model.out[!colSums(is.na(model.out)) > 0,]

#  This plot takes a really long time to plot.

ggplot(model.out, aes(x = dissim, y = preds, group = site, color = init)) +
